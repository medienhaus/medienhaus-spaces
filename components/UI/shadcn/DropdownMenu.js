...
const DropdownMenuItem = React.forwardRef(({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Item
        ref={ref}
        className={cn(
            'relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 outline-none transition-colors focus:bg-accent focus:text-white data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_a]:no-underline [&_a]:focus:text-white',
            inset && 'pl-8',
            className,
        )}
        {...props}
...
const DropdownMenuShortcut = ({ className, ...props }) => {
    return <span className={cn('text ml-auto opacity-60', className)} {...props} />;
};

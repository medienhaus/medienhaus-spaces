...
export default function TreeView({ parsedData, onClick }) {
    ...
    const Branch = ({ name, roomId, type, template }) => {
        return (
            <BranchLayout
    ...
    return (
        ...
        >
            <Branch name={data.name} roomId={data.id} />
            {data.children && (
                <Leaf>
                    {' '}
                    {data.children.map((child, index) => {
                        ...
                        return <TreeView key={index} parsedData={child} onClick={onClick} />;
                    })}
                </Leaf>
            )}

...
const AddExistingContext = ({ parentId, parentName, contextRootId }) => {
    const auth = useAuth();
    ...
    return (
        <Form>
            ...
            { isAddingAllowed && <>
                <button disabled={!isAddingAllowed} onClick={addContextToParent}>add</button>
                <button onClick={cleanUp}>{ t('cancel') }</button>
            </> }

...
function TldrawMatrixProvider(roomId) {
    ...
    useEffect(() => {
        ...
        setMatrixRoomClient(
            new MatrixSpecificRoomsProvider(
    ...
    useEffect(() => {
        ...
        const fetchStoreElementFromStateEvents = async () => {
            const tempStore = {};
            const stateStoreSchema = await matrixRoomClient
                .getMatrixClient()
                .getStateEvent(roomId, 'dev.medienhaus.tldraw.store.schema', '');
            ...
            allTimelineMessages.forEach((message) => {
                ...
                if (message?.content?.msgtype === 'dev.medienhaus.tldraw.store.store') {
                    ...
                    if (
                        ...
                        !allTimelineMessages.find((m) => m.event_id === message.content['m.relates_to'].event_id)?.content?.msgtype
                    )
                        return;

...
function useMatrixProvider(auth) {
    ...
    const assignAccessTokenToAuthProvider = useCallback((eventContent) => {
        const activeAuthProviders = auth.getActiveAuthenticationProviderNames();
        Object.entries(eventContent).forEach(([key, value]) => {
            if (activeAuthProviders.includes(key)) { // we check if the given accessToken from the accountdata is an active authProvider of this current interface
                auth.getAuthenticationProvider(key).setToken(value);
                auth.updateConnectionStatus(key, true); // we assume that this new accessToken is valid and set the connection status to true
            }
        });
    }, [auth]);

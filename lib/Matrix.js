...
function useMatrixProvider(auth) {
    ...
    const hydrateBookmarks = useCallback((bookmarks) => {
    ...
    const AccountDataEvent = useCallback(/** @param {MatrixEvent} event */ (event) => {
        switch (event.getType()) {
            ...
            case 'dev.medienhaus.spaces.bookmarks':
                if (Object.keys(event.getContent()).length > 0) {
                    hydrateBookmarks(event.getContent());
                }
    ...
    }, [assignAccessTokenToAuthProvider, hydrateBookmarks, hydrateDirectMessages]);
        ...
        (newState, prevState) => {
            ...
            if (newState === 'SYNCING' && prevState === 'PREPARED') {
                (async () => {
                    ...
                    // const medienhausBookmarks = await matrixClient.getAccountDataFromServer('dev.medienhaus.spaces.bookmarks');
                    // hydrateBookmarks(medienhausBookmarks);

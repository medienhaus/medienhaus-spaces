...
function useMatrixProvider(auth) {
    ...
    const [knockedMembers, setKnockedMembers] = useImmer(new Map());
    ...
    const buildRoomObject = useCallback((roomId) => {
        ...
        // @TODO this should probably not be necessary, but it is atm...
        if (!room) return;
        ...
        const knockedMembers = room.getMembersWithMembership('knock');

        if (knockedMembers.length > 0) {
            // if there are any knocking members, we add the name of the room to the object and add it to the state
            knockedMembers.forEach((member) => {
                member.roomName = room.name;
            });
            setKnockedMembers(map => {
                map.set(roomId, ...knockedMembers);
            });
        }
    ...
    }, [matrixClient, setKnockedMembers]);
    ...
    const RoomTimelineEvent = useCallback((event, room) => {
        ...
        if (knockedMembers.has(room.roomId) && event.event.content.membership !== 'knock') {
            setKnockedMembers(map => {
    ...
    }, [directMessages, knockedMembers, setDirectMessages, setKnockedMembers, setMessages, setRoom, setRoomContents, setRooms, setSpaces]);

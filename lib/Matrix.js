    ...
    return mapValues(activeMatrixAuthenticationProviders, (authenticationProvider, type) => ((authenticationProvider) => {
        ...
        const buildRoomObject = useCallback((roomId) => {
            ...
            if (room.currentState.events.has('m.space.child')) {
                ...
                const eventArray = Array.from(room.currentState.events.get('m.space.child').values());
                for (const child of eventArray) {
        ...
        const leaveRoom = async (roomId) => {
            const leaveMatrixRoom = await matrixClient.leave(roomId).catch((err) => console.log(err.message));
        ...
        const addSpaceToState = useCallback((roomId) => {
            if (spaces.has(roomId)) return;

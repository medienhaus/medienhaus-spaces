    ...
    return _.mapValues(activeMatrixAuthenticationProviders, (authenticationProvider, type) => ((authenticationProvider) => {
        ...
        useEffect(() => {
            const lookForApplicationsFolder = async () => {
                ...
                console.debug('Trying to find "Applications" space...', { spacesToScan });

                if (existingApplicationsSpace) {
                    console.debug('Found "Applications" space', { existingApplicationsSpace });
                    setApplicationsFolder(existingApplicationsSpace.roomId);
                } else {
                    console.debug('Creating new root "Applications" space...');
                    const newApplicationsSpace = await createRoom(
                        ...
                        'context',
                        'applications');
                    console.debug('Created new "Applications" space', { newApplicationsSpace });
            ...
            const lookForServiceSpaces = async () => {
                ...
                for (const element of Object.keys(getConfig().publicRuntimeConfig.authProviders)) {
                    ...
                    if (existingServiceSpace) {
                        console.debug('Found existing service space', { serviceType: element, existingServiceSpace });
                        serviceSpaces[element] = existingServiceSpace.roomId;
                    } else {
                        console.debug('Creating new service space...', { serviceType: element });
                        const roomId = await createRoom(
                            ...
                            'context',
                            'application');
                        console.debug('Created new service space...', { serviceType: element, roomId });

...
function useMatrixProvider(auth) {
    ...
    const hydrateFavourites = useCallback((favourites) => {
        setFavourites(favourites.favourites);
    }, []);

    const hydrateOnboarding = useCallback((eventContent) => {
        setOnboardingData(eventContent);
    }, []);
    ...
    const AccountDataEvent = useCallback(
        /** @param {MatrixEvent} event */ (event) => {
            switch (event.getType()) {
                ...
                case 'dev.medienhaus.spaces.favourites':
                    if (Object.keys(event.getContent()).length > 0) {
                        hydrateFavourites(event.getContent());
                    }

                    break;

                case 'dev.medienhaus.spaces.onboarding':
                    if (Object.keys(event.getContent()).length > 0) {
                        hydrateOnboarding(event.getContent());
                    }
        ...
        [assignAccessTokenToAuthProvider, hydrateFavourites, hydrateDirectMessages],
    );
    ...
    /**
     * This function is used to create a new room or space in the Matrix client.
     * It uses the authentication provider to create the room and then updates the state accordingly.
     *
     * @callback
     * @async
     * @param {string} name - The name of the room or space to be created.
     * @param {boolean} isSpace - A boolean indicating whether the room is a space.
     * @param {string} topic - The topic of the room or space.
     * @param {string} joinRule - The rule for joining the room or space.
     * @param {string} type - The type of the room or space.
     * @param {string} template - The template for the room or space.
     * @param {string} parentId - The ID of the parent room or space.
     * @returns {Promise<string>} - Returns a promise that resolves to the ID of the created room or space.
     */
    const createRoom = useCallback(
        async (name, isSpace, topic, joinRule, type, template, parentId, visibility, historyVisibility, powerLevelContentOverride) => {
            const room = await authenticationProvider
                .createRoom(
                    name,
                    isSpace,
                    topic,
    ...
    useEffect(() => {
        ...
        const lookForApplicationsFolder = _.debounce(async () => {
            ...
            if (existingApplicationsSpace) {
                ...
                const createNewApplicationsSpace = async () =>
                    await createRoom(
                        'Applications',
                        true,
        ...
        const lookForServiceSpaces = _.debounce(async () => {
            ...
            for (const element of Object.keys(getConfig().publicRuntimeConfig.authProviders)) {
                ...
                if (existingServiceSpace) {
                    ...
                    const createNewServiceRoom = async () =>
                        await createRoom(
                            element,
                            true,
                    ...
                    const addServiceRoomToParent = async () =>
                        await authenticationProvider.addSpaceChild(applicationsFolder, roomId).catch((error) => {
                            return handleRateLimit(error, () => addServiceRoomToParent()).catch((error) => logger.debug(error.message));
                        });
    ...
    const addSpaceChild = async (parent, child, suggested) => {
        return authenticationProvider.addSpaceChild(parent, child, suggested).catch((error) => {

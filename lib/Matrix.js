...
function useMatrixProvider(activeMatrixAuthenticationProviders) {
    return _.mapValues(activeMatrixAuthenticationProviders, (authenticationProvider, type) => ((authenticationProvider) => {
        ...
        const buildRoomObject = useCallback((roomId, metaEvent, inviter) => {
            ...
            if (inviter) roomObject.inviter = inviter;

            return roomObject;
        });

        const setRoom = (roomId, metaEvent, inviter) => {
            return (prevState) => {
                const newState = new Map(prevState);
                if (buildRoomObject(roomId, metaEvent, inviter)) {
                    newState.set(roomId, buildRoomObject(roomId, metaEvent, inviter));
                } else {
        ...
        const RoomMyMembershipEvent = useCallback(async (room, membership, prevMembership) => {
            ...
            if ((membership === 'invite')) {
                ...
                setInvites(setRoom(room.roomId, metaEvent, inviter));
            }

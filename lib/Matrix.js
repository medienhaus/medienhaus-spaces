...
function useMatrixProvider(activeMatrixAuthenticationProviders) {
    return _.mapValues(activeMatrixAuthenticationProviders, (authenticationProvider, type) => ((authenticationProvider) => {
        ...
        const buildRoomObject = useCallback((roomId, metaEvent, inviterName) => {
            ...
            if (inviterName) roomObject.inviterName = inviterName;

            return roomObject;
        });

        const setRoom = (roomId, metaEvent, inviterName) => {
            return (prevState) => {
                const newState = new Map(prevState);
                if (buildRoomObject(roomId, metaEvent, inviterName)) {
                    newState.set(roomId, buildRoomObject(roomId, metaEvent, inviterName));
                } else {
        ...
        const RoomMyMembershipEvent = useCallback(async (room, membership, prevMembership) => {
            ...
            if ((membership === 'invite')) {
                // we need to update the matrixClient room sync so getRooms(roomId) in buildRoomObject doesn't return null
                await matrixClient.roomInitialSync(room.roomId);
                // it seems that invitations don't list custom state events, therefor we need to fetch dev.medienhaus.meta manually
                const metaEvent = await hydrateMetaEvent(room.roomId);
                // we check to see who invited the current user
                const inviterName = room.getMember(matrixClient.getUserId())?.events?.member?.getSender?.();
                setInvites(setRoom(room.roomId, metaEvent, inviterName));
            }

            if (prevMembership === 'invite') {
                // in this case we know the invitation was handled by the user and we can delete it from our invitations
                setInvites(deleteElement(room.roomId));
            }
        });
        ...
        useEffect(() => {
            const lookForServiceSpaces = async () => {
                ...
                for (const element of Object.keys(getConfig().publicRuntimeConfig.authProviders)) {
                    ...
                    if (element === 'matrixContentStorage') continue;
        ...
        const createRoom = async (name, isSpace, topic, joinRule, type, template, application, visibility, historyVisibility, preset) => {
            const room = await authenticationProvider.createRoom(name, isSpace, topic, joinRule, type, template, application, visibility, historyVisibility, preset);
            if (isSpace) {
        ...
        const hydrateMetaEvent = async (roomId, abortSignal) => {
            // If for some reason this function was called without a valid roomId we just cancel right away
            if (!roomId) return;
            // And if we already have the metaEvent of the given room we also cancel right away
            if (rooms.get(roomId)?.meta !== undefined) return;
            if (spaces.get(roomId)?.meta !== undefined) return;

            const fetchMetaEvent = await authenticationProvider.fetchMetaEvent(roomId, 1, abortSignal);

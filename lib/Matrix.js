    ...
    return _.mapValues(activeMatrixAuthenticationProviders, (authenticationProvider, type) => ((authenticationProvider) => {
        ...
        useEffect(() => {
            const lookForApplicationsFolder = async () => {
                const findApplicationsFolder = Array.from(spaces.values()).find(space => space.meta?.template === 'applications');
                if (findApplicationsFolder) { //return spaceId
                    // eslint-disable-next-line no-undef
                    if (process.env.NODE_ENV === 'development') console.log('found applications space');
                    setApplicationsFolder(findApplicationsFolder.roomId);
                } else { // create Applications folder
                    // eslint-disable-next-line no-undef
                    if (process.env.NODE_ENV === 'development') console.log('creating root applications folder');
                    const newApplicationsFolder = await createRoom(
                        'Applications',
                        true,
                        ...
                        'context',
                        'applications');
                    setApplicationsFolder(newApplicationsFolder);
                }
            };

            if (initialSyncDone) {
                lookForApplicationsFolder();
                // force bubble layout on users
                let mxLocalSettings = JSON.parse(localStorage.getItem('mx_local_settings'));
                if (mxLocalSettings) mxLocalSettings.layout = 'bubble';
                else mxLocalSettings = { layout: 'bubble' };
            }
        ...
        useEffect(() => {
            const lookForServiceSpaces = async () => {
                ...
                for (const element of Object.keys(getConfig().publicRuntimeConfig.authProviders)) {
                    ...
                    const findServiceSpace = Array.from(spaces.values()).find(space => space.name === element);
                    if (findServiceSpace) serviceSpaces[element] = findServiceSpace.roomId;
                    else {
                ...
                const findServiceSpace = Array.from(spaces.values()).find(space => space.name === 'bookmarks');
                if (findServiceSpace) serviceSpaces['bookmarks'] = findServiceSpace.roomId;
                else {
                    console.info('creating bookmark space');
                    const roomId = await createRoom(
                        'bookmarks',
                        true,
            ...
            return () => {
                console.log('UNMOUNT services hook');
            };
        }, [applicationsFolder]);
        ...
        const hydrateRoomContent = async (roomId, limit, signal) => {
            ...
            const fetchMessages = await authenticationProvider.fetchRoomMessages(roomId, limit, signal);

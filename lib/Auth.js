...
function useAuthProvider() {
    ...
    const [connectionStatus, setConnectionStatus] = useState({ 'etherpad': false, 'spacedeck': false });
    ...
    const signout = useCallback(async () => {
        ...
        await getAuthenticationProvider('matrix').getMatrixClient().deleteAccountData('dev.medienhaus.spaces.accesstokens');

        // 3. Sign out of Matrix
        await getAuthenticationProvider('matrix').getMatrixClient().logout(true);

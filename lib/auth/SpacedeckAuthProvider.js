...
class SpacedeckAuthProvider {
    ...
    async fetchCallWithLoginPrompt(request, options, maxTry = 1) {
        ...
        if (response.status === 401 || response.status === 403) {
            ...
            const password = prompt('Please re-enter your password ' + username);
            if (password) {
                const signIn = await this.signin(username, password);
                options.headers['X-Spacedeck-Auth'] = signIn;
                if (signIn) return await this.fetchCallWithLoginPrompt(request, options, maxTry + 1);
            } else throw new Error('Password not provided.');
        } else return response;
    }

    async _getInititalUserData() {
        console.log('_getInititalUserData');
    ...
    async _login(baseUrl, usr, password) {
        const response = await this.fetchCallWithLoginPrompt(baseUrl + '/api/sessions', {
            method: 'POST',
            body: JSON.stringify({
                email: usr,
                password: password,
    ...
    async deleteSpaceById(spaceId) {
        ...
        return await this.fetchCallWithLoginPrompt(this.url + '/api/spaces/' + spaceId, {
            method: 'DELETE',
            headers: {
                'x-spacedeck-auth': this.token,
            },

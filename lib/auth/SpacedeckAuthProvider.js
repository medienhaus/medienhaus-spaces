...
class SpacedeckAuthProvider {
    ...
    async validateToken() {
        ...
        this.updateConnectionStatus('spacedeck', true);

        return tokenValidationCall._id;
    }
    ...
    async _createElement(name, parentFolderId = this.homeFolderId, type = 'space') {
        const response = await this.makeRequest(this.url + '/api/spaces', {
            ...
            body: JSON.stringify({
                ...
                /*
                it is not possible to pass the visibility as a create attribute in the body (https://github.com/spacedeck/spacedeck-open/blob/mnt/routes/api/spaces.js#L188), 
                so we have to do change the default visibility to 'public' in an addtional call. 
                */
            }),
        });
        ...
        const visibilityResponse = await this._changeVisibility(content?._id,content,'public')
        console.log('b')


        if(visibilityResponse?.access_mode === 'public') { //we check if the request was successful based on the return from the spacedeck backend
            return content;
        } else {
            return
        }
    ...
    async _changeVisibility(sketchId,baseContent, visibility) {
        baseContent.access_mode =  (visibility === 'public' || visibility === 'private') ? visibility : 'public'     // check if the visibility is one of the default strings otherwise fallback to default which should be 'public'
        baseContent.editors_locking = (visibility === 'public' || visibility === 'private') ? (visibility === 'public' ? true : false) : true   // check if the visibility is one of the default strings otherwise fallback to default which should be true
     

        console.log(baseContent)
        const response = await this.makeRequest(this.url + '/api/spaces/'+sketchId , {
            method: 'PUT',
            headers: {
                'X-Spacedeck-Auth': this.token,
                'Content-Type': 'application/json',
            ...
            //we need to submit all data to change just one attribute of a given sketch. the spacedeck api is weird in that way.
        }); 

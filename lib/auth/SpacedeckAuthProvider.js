...
class SpacedeckAuthProvider {
    ...
    async fetchCallWithLoginPrompt(request, options, maxTry = 1) {
        ...
        if (response.status === 401 || response.status === 403) {
            ...
            if (password) {
                const sign = await this.signin(username, password);
                options.headers['X-Spacedeck-Auth'] = sign;
                if (sign) await this.fetchCallWithLoginPrompt(request, options, maxTry + 1);
    ...
    async _login(baseUrl, usr, password) {
        //@TODO add maximun tries for login

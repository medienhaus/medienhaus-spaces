...
class SketchAuthProvider {
    ...
    async initByToken(url = this.url, token = this.token) {
    ...
    _updateSpace(space) {
        if (!this.getSpaceById(space._id)) this.getAllSpaces[space.id] = space;
    // adding replace in tree is missing
    }

    _updateFolder(folder) {
        if (!this.getFolderById(folder._id)) this.getAllFolder[folder.id] = folder;
    // adding replace in tree is missing
    }
    ...
    async createFolder(folderName, parentFolderId = this.homeFolderId) {
        const data = this._createElement(folderName, parentFolderId, 'folder');
        if (!data) return;
        this.allFolder[data._id] = data;
        // adding to structure missing
        return data;
    }
    ...
    // timeout idk why.
        if (!this.getFolderById(folderId)) return;
        const response = await fetch(this.url + 'api/spaces/' + folderId, {
            method: 'delete',
            headers: {
                'X-Spacedeck-Auth': this.token,
            },
    ...
    async setVisibilityOfSpaceById(spaceId, visibility) {
        const space = this.getSpaceById(spaceId);
        if (!space) return;
        space.access_mode = visibility === 'public' ? 'public' : 'private';

        const response = await fetch(this.url + 'api/spaces/' + space._id, {
            method: 'put',
            headers: {
                'X-Spacedeck-Auth': this.token,
                'Content-Type': 'application/json',
        ...
        space.updated_at = data?.updated_at;
        space.updatedAt = data?.updatedAt;

        this._updateSpace(space);
        return space;
    }

    async setEditorLockingOfSpaceById(spaceId, lock) {
        const space = this.getSpaceById(spaceId);
        if (!space) return;
        if (typeof lock !== 'boolean') return;
        space.editors_locking = lock;

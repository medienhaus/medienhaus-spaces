...
class MatrixAuthProvider {
     ...
     * @param {Object} powerLevelContentOverride - The power level content override for the room or space.
     ...
     */
    async createRoom(name, isSpace, topic, joinRule, type, template, parentId, visibility, historyVisibility, powerLevelContentOverride) {
        const opts = {
            name: name,
            preset: 'private_chat',
            topic: topic,
    ...
    knockOnRoom(roomId) {
        return this.matrixClient.http.authedRequest('POST', `/knock/${roomId}`, undefined, {});
    }
}
